...
use pktparse::arp::{parse_arp_pkt, ArpPacket};
use pktparse::ethernet::{parse_ethernet_frame, EtherType, EthernetFrame};
use pktparse::ip::IPProtocol;
use pktparse::ipv4::{parse_ipv4_header, IPv4Header};
use pktparse::ipv6::{parse_ipv6_header, IPv6Header};
...
use tls_parser::tls::{parse_tls_encrypted, TlsEncrypted};
use tls_parser::tls::{parse_tls_plaintext, TlsPlaintext};
...
impl<'a> ParsedPacket<'a> {
    pub fn new(
        networkacces: EthernetFrame,
        internet: InternetLayer,
        transport: TransportLayer,
        presentation: PresentationLayer<'a>,
...
impl<'a> Parser<'a> {
    ...
    fn parser_ipv4(
        ...
        data: &'a [u8],
        networkacces: EthernetFrame,
    ) -> Result<ParsedPacket<'a>, String> {
        //parse IPv4 protocol
        match parse_ipv4_header(data) {
            Ok((dat, internet)) => match internet.protocol {
                IPProtocol::UDP => {
                    let actualpacket =
                        self.parser_udp(dat, networkacces, InternetLayer::IPv4(internet));
                    actualpacket
                }
                IPProtocol::TCP => {
                    let actualpacket =
                        self.parser_tcp(dat, networkacces, InternetLayer::IPv4(internet));
                    actualpacket
                }
                _ => {
                    let transport = TransportLayer::None;
    ...
    fn parser_ipv6(
        ...
        match parse_ipv6_header(data) {
            Ok((dat, internet)) => match internet.next_header {
                IPProtocol::UDP => {
                    let actualpacket =
                        self.parser_udp(dat, networkacces, InternetLayer::IPv6(internet));
                    actualpacket
                }
                IPProtocol::TCP => {
                    let actualpacket =
                        self.parser_tcp(dat, networkacces, InternetLayer::IPv6(internet));
                    actualpacket
                }
                _ => {
                    let transport = TransportLayer::None;
        ...
        match parse_tcp_header(data) {
            Ok((dat, transport)) => match transport.dest_port {
                ...
                _ => {
                    let application = ApplicationLayer::None;
                    let actualpacket =
                        self.parser_tls_only(dat, networkacces, internet, transport, application);
                    actualpacket
                }
            },
            ...
            Ok((dat, transport)) => {
                match transport.dest_port {
                    ...
                    _ => match transport.source_port {
                        53 => {
                            let actualpacket =
                                self.parser_dns(dat, networkacces, internet, transport);
                            actualpacket
                        }
                        5353 => {
                            let actualpacket =
                        ...
                        _ => match RtpReader::new(dat) {
                            Ok(application) => {
                                ...
                                let actualpacket = ParsedPacket::new(
                                    networkacces,
                                    internet,
                                    TransportLayer::UDP(transport),
                                    presentation,
                                    ApplicationLayer::RTP(application),
                                );
                                Ok(actualpacket)
                            }
                            Err(_err) => {
    ...
    fn parser_tls(
        ...
        match parse_tls_plaintext(data) {
            Ok((dat, presentation)) => {
                let actualpacket = self.parser_http(
                    dat,
                    networkacces,
                    internet,
                    transport,
                    PresentationLayer::TLSP(presentation),
                );
                actualpacket
            }
            Err(_err) => match parse_tls_encrypted(data) {
                Ok((dat, presentation)) => {
                    let actualpacket = self.parser_http(
                        ...
                        internet,
                        transport,
                        PresentationLayer::TLSE(presentation),
                    );
                    actualpacket
                }
                Err(_err) => {
                    let presentation = PresentationLayer::None;
                    let actualpacket =
                        self.parser_http(data, networkacces, internet, transport, presentation);
                    actualpacket
                }
            },
        }
        ...
        match parse_chunk_size(data) {
            Ok(application) => {
                let actualpacket = ParsedPacket::new(
                    ...
                    TransportLayer::TCP(transport),
                    presentation,
                    ApplicationLayer::HTTPchunck(application),
                );
                Ok(actualpacket)
            }
            Err(_err) => {
        ...
        match Packet::parse(data) {
            Ok(application) => {
                ...
                let actualpacket = ParsedPacket::new(
                    ...
                    TransportLayer::UDP(transport),
                    presentation,
                    ApplicationLayer::DNS(application),
                );
                Ok(actualpacket)
            }
            Err(err) => {
        ...
        match parse_arp_pkt(data) {
            Ok((dat, application)) => {
                ...
                let actualpacket = ParsedPacket::new(
                    ...
                    transport,
                    presentation,
                    ApplicationLayer::ARP(application),
                );
                Ok(actualpacket)
            }
            Err(err) => {
                ...
                let application = ApplicationLayer::None;
                let actualpacket =
                    ParsedPacket::new(networkacces, internet, transport, presentation, application);
                Ok(actualpacket)
            }
        }
    }
    ...
    pub fn parse_packet(&self) -> Result<ParsedPacket<'a>, String> {
        match self.parser_ethernet(self.data) {
            ...
            Err(err) => Err("Non si può parsare questo pacchetto".to_string()),
        }
    }
}
